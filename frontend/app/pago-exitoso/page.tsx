...
type OrdenPublica = {
  _id: string;
  total: number;
  estadoPago: "pendiente" | "pagado" | "fallido" | "reembolsado" | string;
  estadoFulfillment?:
    | "pendiente"
    | "procesando"
    | "enviado"
...
/* ======================================================
   Página SSR — Pago Exitoso (ENTERPRISE PRO)
====================================================== */
export default async function PagoExitosoPage(props: {
  ...
  // valor inicial desde query ?ordenId=
  let ordenId: string | undefined = ordenIdParam;

  /* ======================================================
     Fallback profesional: resolver ordenId por session_id
  ====================================================== */
  if (!ordenId && sessionId) {
    try {
      const res = await fetch(
        `${backendUrl}/api/pagos/estado?session_id=${encodeURIComponent(
          sessionId
        )}`,
        { cache: "no-store" }
      );
      ...
      // silencio: si falla, se manejará abajo como orden no identificada
    }
  }
  ...
  const apiUrl = `${backendUrl}/api/ordenes/public/${encodeURIComponent(
    ordenId
  )}`;
      ...
      <div style={{ display: "grid", gap: 16 }}>
        <Card>
          ...
          <p style={{ marginTop: 10, opacity: 0.9 }}>
            {isPendiente
              ? "Estamos verificando tu pago con Stripe. No cierres esta ventana, se actualizará automáticamente."
          ...
          >
            ...
            <Badge>Total: ${Number(orden.total).toFixed(2)}</Badge>
